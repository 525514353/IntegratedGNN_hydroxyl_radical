...
def data_distribution():
    ...
    plt.xlabel('Value', fontsize=18, fontweight='bold')  # 调整字号
    plt.ylabel('Frequency', fontsize=18, fontweight='bold')  # 调整字号
    plt.title('Histogram of Values', fontsize=18, fontweight='bold')  # 调整字号和标题
    plt.grid(True)  # 添加网格线
    plt.tight_layout()  # 调整图表布局，防止标签截断
    plt.show()


    # 设置图表风格为科技论文风格
    sns.set_context("paper")  # 设置为纸张大小的风格
    sns.set_style("whitegrid")  # 设置白色网格背景

    # 绘制小提琴图
    plt.figure(figsize=(8, 6),dpi=300)  # 设置图表大小
    sns.violinplot(x, inner='quart', linewidth=2, palette='pastel')  # 绘制小提琴图

    # 添加坐标轴标签和标题
    plt.xlabel('proportion', fontsize=16, fontweight='bold')  # 调整 x 轴标签字号和加粗
    plt.ylabel('Value', fontsize=16, fontweight='bold')  # 调整 y 轴标签字号和加粗
    plt.title('Distribution of Values', fontsize=16, fontweight='bold')  # 调整标题字号和加粗

    # 添加水平线，表示中位数
    median_value = np.median(x)  # 计算中位数
    plt.axhline(y=median_value, color='red', linestyle='--', linewidth=2, label='Median')  # 添加中位数水平线
    plt.yticks(fontsize=14)
    # 添加图例
    plt.legend(fontsize=14)  # 调整图例字号

    # 调整图表布局，防止标签截断
    plt.tight_layout()

    # 保存图表（可选）
    # plt.savefig('violin_plot.png', dpi=300)  # 将图表保存为高分辨率图片
